...
export function ProjectCard({ project, index = 0 }: { project: Project; index?: number }) {
  ...
  return (
    <motion.article
      ...
      className="group relative flex h-full flex-col overflow-hidden rounded-3xl border border-white/10 bg-surface/90 backdrop-blur-md transition-shadow hover:border-white/20 hover:shadow-[0_24px_60px_rgba(0,0,0,0.45)]"
      data-cursor="view"
    >
      ...
      <div className="flex flex-1 flex-col p-6 md:p-8">
        ...
        <div className="mt-5 flex flex-wrap gap-2">
          {project.stack.slice(0, 4).map((t) => (
            <span
              key={t}
              className="rounded-full border border-white/10 bg-white/[0.03] px-2.5 py-1 text-[10px] font-medium text-slate-400"
            >
              {t}
            </span>
          ))}
